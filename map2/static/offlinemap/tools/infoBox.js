     ...
     baidu.object.extend(InfoBox.prototype,{
         ...
         _adjustPosition: function(poi){
             ...
             switch(this._opts.align){
                 case INFOBOX_AT_TOP:
                     if(this._marker){
                         this._div.style.bottom = -(pixel.y - this._opts.offset.height - icon.anchor.height + icon.infoWindowOffset.height) - this._marker.getOffset().height + 2 + "px";
                     }else{
                 ...
                 case INFOBOX_AT_BOTTOM:
                     if(this._marker){
                           this._div.style.top = pixel.y + this._opts.offset.height - icon.anchor.height + icon.infoWindowOffset.height + this._marker.getOffset().height + "px";
                     }else{
             ...
             if(this._marker){
                 this._div.style.left = pixel.x - icon.anchor.width + this._marker.getOffset().width + icon.infoWindowOffset.width - this._boxWidth / 2 + this._opts.offset.width + "px";
             }else{
         ...
         _panBox: function(){
             ...
             switch(this._opts.align){
                 case INFOBOX_AT_TOP:
                     //上侧超出
                     var h = this._marker ? icon.anchor.height + this._marker.getOffset().height - icon.infoWindowOffset.height : 0;
                     panTop = boxH - anchorPos.y + this._opts.offset.height + h + 2 ;
                     break;
                 case INFOBOX_AT_BOTTOM:
                     //下侧超出
                     var h = this._marker ? -icon.anchor.height + icon.infoWindowOffset.height + this._marker.getOffset().height + this._opts.offset.height : 0;
